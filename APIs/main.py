...
def handle_booking():
    ...
    logger.info(payload['data'].get('firstname'))
    ...
    #required_fields = ['firstname', 'lastname', 'num_people', 'booking_datetime']
    #if not all(field in data for field in required_fields):
    #    return jsonify({'error': 'Missing required fields'}), 400

    # Validate date format
    #try:
    #    datetime.strptime(data["data"].get('booking_datetime'), '%Y-%m-%d %H:%M:%S')
    #except ValueError:
    ...
    if request.method == 'POST':
        ...
        try:
            c.execute('''
                ...
                VALUES (?, ?, ?, ?)
            ''', (data["data"].get('firstname'), data["data"].get('lastname'), data["data"].get('num_people'), data["data"].get('booking_datetime')))
            conn.commit()
